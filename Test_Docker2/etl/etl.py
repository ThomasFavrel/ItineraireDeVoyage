...
def dataframeFromJson():
    ...
    pd.json_normalize(list(client.poi_db.poi_col.find({})), errors='ignore')[col_select_raw]
...
def main():
    df_raw = dataframeFromJson()
    df_preprocessed, df_theme_unique, df_theme = preprocessData(df_raw)
    df_preprocessed.to_csv("/app2/Data/POI.csv")
    df_theme.to_csv("/app2/Data/theme.csv")
    df_theme_unique.to_csv("/app2/Data/theme_unique.csv")
